...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private  void init()
    {
        ivDinamic = findViewById(R.id.iv_dynamic);
        tvDegree = findViewById(R.id.tvDegre);
        sensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);
    }

    /**
     * 1) аргумент - кто прослушивает
     * 2) аргумент - указываем какой сенсор будет использоваться (в нашем случаее сенсор который отвечает за ориентацию)
     * 3) указываем задержку*/
    @Override
